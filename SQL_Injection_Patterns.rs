...
fn check_for_sql_injection(query: &str) -> bool {
    let sql_injection_patterns = vec![
        ...
        r"(?i)\bselect\b.*\bhmac\b",  // SELECT HMAC 패턴
        r"(?i)\bselect\b.*\bsha\b",  // SELECT SHA 패턴
        r"(?i)\bselect\b.*\bmd5\b",  // SELECT MD5 패턴
        r"(?i)\bselect\b.*\bsha1\b",  // SELECT SHA1 패턴
        r"(?i)\bselect\b.*\bsha256\b",  // SELECT SHA256 패턴
        r"(?i)\bselect\b.*\bsha512\b",  // SELECT SHA512 패턴
        r"(?i)\bselect\b.*\bripemd\b",  // SELECT RIPEMD 패턴
        r"(?i)\bselect\b.*\bripemd160\b",  // SELECT RIPEMD160 패턴
        r"(?i)\bselect\b.*\bblake\b",  // SELECT BLAKE 패턴
        r"(?i)\bselect\b.*\bblake2\b",  // SELECT BLAKE2 패턴
        r"(?i)\bselect\b.*\btiger\b",  // SELECT TIGER 패턴
        r"(?i)\bselect\b.*\bwhirlpool\b",  // SELECT WHIRLPOOL 패턴
        r"(?i)\bselect\b.*\bhash\b",  // SELECT HASH 패턴
        r"(?i)\bselect\b.*\bhashes\b",  // SELECT HASHES 패턴
        r"(?i)\bselect\b.*\bdigest\b",  // SELECT DIGEST 패턴
        r"(?i)\bselect\b.*\bdigests\b",  // SELECT DIGESTS 패턴
        r"(?i)\bselect\b.*\bsignature\b",  // SELECT SIGNATURE 패턴
        r"(?i)\bselect\b.*\bsignatures\b",  // SELECT SIGNATURES 패턴
        r"(?i)\bselect\b.*\bcert\b",  // SELECT CERT 패턴
        r"(?i)\bselect\b.*\bcerts\b",  // SELECT CERTS 패턴
        r"(?i)\bselect\b.*\bcertificate\b",  // SELECT CERTIFICATE 패턴
        r"(?i)\bselect\b.*\bcertificates\b",  // SELECT CERTIFICATES 패턴
        r"(?i)\bselect\b.*\bpem\b",  // SELECT PEM 패턴
        r"(?i)\bselect\b.*\bpkcs\b",  // SELECT PKCS 패턴
